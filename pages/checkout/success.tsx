...
// Assuming BaseOrderItemData is similar to what ApiOrderItem needs for its fields
// If IOrderItemData from types/orderTypes.ts is suitable for ApiOrderItem's fields, use it.
// For clarity, ApiOrderItem is defined below.
// import { IOrderItemData as BaseOrderItemData } from '@/types/orderTypes';
...
interface ApiOrderItem {
    ...
    image?: string;
    productId?: string | { _id: string; name: string; /* ... other populated fields */ };
}
...
const CheckoutSuccessPage: React.FC = () => {
  ...
  const [orderDetails, setOrderDetails] = useState<OrderConfirmationDisplayData | null>(null);
  ...
  useEffect(() => {
    if (order_id_internal && typeof order_id_internal === 'string') {
      ...
      fetch(`/api/orders/confirmation/${order_id_internal}`)
        .then(res => {
          if (!res.ok) {
            // Assuming errData is { message: string }
            return res.json().then((errData: { message?: string }) => {
              throw new Error(errData.message || 'Failed to fetch order details for confirmation.');
            });
          }
          return res.json();
        })
        .then(data => { // Assuming data is { order: YourFullOrderDataTypeFromApi }
          if (data.order) {
            // FIXED: Error at 68:60 - Typed 'item' in map using ApiOrderItem
            const displayData: OrderConfirmationDisplayData = {
                orderId: data.order.orderId,
                customerDetails: { firstName: data.order.customerDetails?.firstName },
            ...
            setError('Could not retrieve order confirmation details at this time.');
          }
        })
      ...
      setError("Order confirmation ID missing. Your payment was likely successful; please check your email.");
      setIsLoading(false);
    }
    ...
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-8rem)] bg-gray-50 p-4 sm:p-6 lg:p-8 font-sans text-gray-800">
      ...
      <div className="bg-white p-6 sm:p-10 rounded-xl shadow-2xl max-w-xl w-full text-center">
        <div className="mx-auto flex items-center justify-center h-20 w-20 sm:h-24 sm:w-24 rounded-full bg-green-100 mb-6">
          <svg className="h-10 w-10 sm:h-12 sm:w-12 text-green-500" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor">
        ...
        {isLoading && <p className="text-gray-600 my-6 animate-pulse">Loading your order summary...</p>}
        ...
        {!isLoading && error && !orderDetails && (
          <>
            ...
            <p className="text-gray-500 text-sm mt-2">However, your order is being processed. You will receive an email confirmation shortly with all the details.</p>
          </>
        )}
        ...
        {!isLoading && orderDetails && (
          <>
            <p className="text-lg text-gray-700 my-4">
              Thank you, {orderDetails.customerDetails?.firstName || 'Valued Customer'}! Your order has been successfully placed.
            </p>
            <div className="text-left my-6 p-4 sm:p-6 bg-gray-50 rounded-lg border border-gray-200 space-y-3">
              ...
              <ul className="space-y-2 text-sm text-gray-600">
                {orderDetails.orderItems.map((item: DisplayOrderItem, index: number) => (
            ...
            <p className="text-gray-600 text-sm mt-4">
              A confirmation email with your full order details has been sent to your email address.
              We&apos;ll notify you again once your order is shipped.
            </p>
          </>
        )}

         {!isLoading && !error && !orderDetails && !router.query.order_id_internal && router.isReady && (
             <p className="text-lg text-gray-700 my-4">
