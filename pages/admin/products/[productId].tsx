...
const EditProductPage: React.FC<AdminAuthProps> = ({ adminUser }) => {
  ...
  const fetchProduct = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || `Failed to fetch product: ${res.statusText}`);
      }
      if (data.product) {
  ...
  useEffect(() => {
    if (productId && adminUser) { // Fetch only when productId is available and admin is authenticated
      fetchProduct();
    }
  ...
  const handleUpdateProduct = async (data: Partial<IProductData>) => {
    ...
    try {
        ...
        if (!response.ok) {
            const errorMessage = result.message || (result.errors ? result.errors.map((e: any) => e.message).join(', ') : 'Failed to update product.');
            throw new Error(errorMessage);
        }
        setFormSuccess(`Product "${result.product.name}" updated successfully!`);
        setProduct(result.product); // Update local state with the updated product
        // Optionally, add a small delay then clear success message, or redirect
  ...
  if (isLoading) return <div style={{ padding: '20px', textAlign: 'center' }}>Loading product data...</div>;
  if (error) return <div style={{ padding: '20px', color: 'red', textAlign: 'center' }}>Error: {error} <Link href="/admin/products" legacyBehavior><a>&larr; Back to Product List</a></Link></div>;
  if (!product) return <div style={{ padding: '20px', textAlign: 'center' }}>Product not found. <Link href="/admin/products" legacyBehavior><a>&larr; Back to Product List</a></Link></div>;
