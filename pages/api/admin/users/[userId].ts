...
async function handler(req: NextApiRequestWithAdmin, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case "PUT":
      try {
        const validatedData = AdminUserUpdateSchema.parse(
        ...
        const adminUserToUpdate = await User.findOne({
          _id: userId,
          role: "ADMIN",
        });
        ...
        if (
          validatedData.email &&
          validatedData.email.toLowerCase() !== adminUserToUpdate.email
        ) {
        ...
        if (validatedData.firstName)
          adminUserToUpdate.firstName = validatedData.firstName;
        if (validatedData.lastName)
          adminUserToUpdate.lastName = validatedData.lastName;
        ...
        if (
          ...
          (error as MongooseDuplicateKeyError).keyPattern?.email
        ) {
          return res
            .status(409)
            .json({
              message:
                "This email is already in use by another user (database constraint).",
            });
        }
    ...
    case "DELETE":
      try {
        ...
        if (result.deletedCount === 0) {
          return res
            .status(404)
            .json({ message: "Admin user not found or already deleted." });
        }
